...
| Type | Count | Meaning |
|---|---:|---|
| FULL_IMAGE | at least 2 | Complete generated section image replaces HTML layout; mandatory hero + final product closing |
...
| 99 | final-closing | final closing | image-dominant | FULL_IMAGE | ai-section-designs/99_final-closing.png | assets/inbox/product-front.png | GENERATE_WITH_REFERENCE | one image section | mandatory product/result closing image, no CTA text |
...
- final product/result closing full-image section

These rows must be present in `image-plan.md`. If mutable legal, price, option, compatibility, or FAQ copy must remain editable, keep that copy in a neighboring `HTML_MIXED`/`HTML_ONLY` section and still generate the separate final closing `FULL_IMAGE` with no purchase-action text.
...
- [ ] `FULL_IMAGE` count is at least 2 and includes the opening hero plus final product/result closing section.
- [ ] The mandatory hero and final closing rows have stable output filenames under `assets/generated/ai-section-designs/`.
- [ ] The final closing row does not contain CTA-button text, option/order prompts, benefit-check prompts, or button-equivalent text.
...
- [ ] Direct numeric prices do not appear in images or visible HTML; final-section price/option/benefit prompts are not used.
...
- mandatory final product/result closing impression
