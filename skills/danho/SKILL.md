---
name: danho
description: Start or initialize the Danho Korean ecommerce 상세페이지 maker workflow with a Korean slash-command-friendly alias. Use when the user invokes /단호한상세페이지, /danho, asks to start the Danho detail-page maker, wants to initialize a new working directory with AGENT.MD, or wants the full product 상세페이지 workflow from planning through PM review, copy review, HTML coding, image planning, GPT Image 2.0 native generation, and final validation.
---
...
4. Preserve all current Danho workflow gates: PM planning loop before copywriter review, copywriter review before Phase A, PM pre-coding review before HTML, HTML before image generation, and built-in Codex `image_gen.imagegen` GPT Image 2.0 native generation only.
5. Ask only for factual blockers. Infer non-critical ecommerce context from the supplied product details.
