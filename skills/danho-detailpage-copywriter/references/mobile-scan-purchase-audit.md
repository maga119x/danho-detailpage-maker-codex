...
- The first 3 sections must answer: what this is, what benefit I get, and what I should check before buying.
- Section copy should support one decision at a time. Do not mix problem, proof, option, and FAQ logic in one headline.
- Product pages need concrete facts, not broad brand claims. Use fit, component, compatibility, limits, review/testimonial, proof, option-area cues, and value framing where relevant. Do not expose direct numeric prices or sales-channel names in visible copy.
- The customer is the hero. The product is the tool or guide that helps the customer act.
- Natural language is not enough. Selling sections must create conversion force through before/after, target desire, value confidence, proof, or a clear next action.
...
| What should I check or do to buy? | Fit, option, component, mutable benefit/option cue, and next action are easy to find before CTA. |
...
| expression_polish | Headlines are complete, collocations are natural, direct prices are absent from visible copy, CTA cues are distinct, repeated action terms are consistent, FAQ answers are direct, the final CTA has a non-button closing/option cue, repeated words are controlled, and buyer-facing terms replace spec/internal terms. | Meaning is clear but one or two phrases are stiff, repetitive, or slightly uneven in rhythm. | Incomplete headlines, direct numeric prices, wrong `체감/느껴짐` subjects, duplicated CTA cues, inconsistent action terms, FAQ non-answers, missing final static CTA cue, spec-sheet terms, production notes, or repeated phrases remain. |
...
| style_consistency | Tone is coherent within each section, repeated action terms are deliberate, lists/cards have matching rhythm, and CTA labels differ by action. | Page-level variation is acceptable but one minor list/term inconsistency remains. | Random `해요/합니다` mixing, terminology drift, broken card/list rhythm, duplicated CTA verbs, or inconsistent emphasis remains. |
...
- For empathy-heavy pages, `empathy_depth` and `purchase_desire` must each be at least 8.0 in the problem, solution, benefit, and CTA sections.
...
- Page-level product identity, buyer benefit, buying/check action, desire, proof/readiness, and native Korean quality must each score at least 9.2.
...
- If expression polish is weak: complete the headline, fix wrong collocations, make CTA cues distinct, unify repeated action terms, fix parallel list rhythm, answer FAQs directly, add a final non-button closing/option cue, replace spec/internal terms, vary repeated wording, and move production notes to proof logs.
...
- If style consistency is weak: choose one vocabulary family per repeated action, align list/card endings, separate `해요체` selling copy from `합니다체` factual copy, and make CTA actions distinct.
- If product is the hero: start with the buyer's home, body, routine, worry, or action.
- If purchase action is unclear: add a fit check, option cue, mutable benefit cue, or CTA cue.
...
- If conversion force is weak: add a target desire, before/after contrast, current-alternative friction, value-stack reason, proof visual, or direct next action.
